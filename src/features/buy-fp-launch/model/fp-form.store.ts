...
export class FPFormLaunchStore {
  ...
  private init = async (): Promise<void> => {
    try {
      const bytes32Symbol = formatBytes32String(TOKEN_SYMBOLS.FP);
  ...
  public get destinationContract(): Contract {
    const token = BLOCKCHAIN[this._rootStore.chain.id]["tokens"][TOKEN_SYMBOLS.FP]
